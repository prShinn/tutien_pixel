const Render = {
  frame() {
    const p = PLAYER;
    if (!p) return;
    GamePlay.cam.x = lerp(GamePlay.cam.x, p.px - cW / 2, 0.18);
    GamePlay.cam.y = lerp(GamePlay.cam.y, p.py - cH / 2, 0.18);
    GamePlay.cam.x = Math.max(
      0,
      Math.min(GamePlay.mapW * GamePlay.frame - cW, GamePlay.cam.x),
    );
    GamePlay.cam.y = Math.max(
      0,
      Math.min(GamePlay.mapH * GamePlay.frame - cH, GamePlay.cam.y),
    );
    const cx = GamePlay.cam.x,
      cy = GamePlay.cam.y;
    ctx.clearRect(0, 0, cW, cH);
    const ts = GamePlay.frame,
      tx0 = Math.max(0, Math.floor(cx / ts)),
      ty0 = Math.max(0, Math.floor(cy / ts));
    const tx1 = Math.min(GamePlay.mapW - 1, tx0 + Math.ceil(cW / ts) + 1),
      ty1 = Math.min(GamePlay.mapH - 1, ty0 + Math.ceil(cH / ts) + 1);
    for (let ty = ty0; ty <= ty1; ty++)
      for (let tx = tx0; tx <= tx1; tx++)
        Render.tile(GamePlay.map.jsonMap[ty][tx], tx * ts - cx, ty * ts - cy);
    for (const portal of GamePlay.map.portals)
      if (portal.label) Render.portal(portal, cx, cy);
    for (const npc of GamePlay.map.npcs) Render.npc(npc, cx, cy);
    // Other players (co-op)
    for (const [, op] of otherPlayers)
      if (op.mapId === GamePlay.mapId) Render.otherPlayer(op, cx, cy);
    for (const m of GamePlay.map?.monsters || [])
      if (!m.dead) Render.monster(m, cx, cy);
  ...
  tile(t, sx, sy) {
    ...
    ctx.fillStyle = c[0];
    ctx.fillRect(sx, sy, GamePlay.frame, GamePlay.frame);
    ctx.fillStyle = c[1];
    const ts = GamePlay.frame;
    if (t === T.GRASS) {
  ...
  npc(npc, cx, cy) {
    ...
    if (dist(PLAYER.px, PLAYER.py, npc.px, npc.py) < GamePlay.frame * 1.6) {
  ...
  portal(portal, cx, cy) {
    const sx = portal.tx * GamePlay.frame + GamePlay.frame / 2 - cx,
      sy = portal.ty * GamePlay.frame + GamePlay.frame / 2 - cy;
  ...
  floatDmg(wx, wy, offY, text, color) {
    ...
    const sx = wx - GamePlay.cam.x + area.left,
      sy = wy - GamePlay.cam.y + offY + area.top;
...
const UI = {
  update() {
    ...
    st.textContent = `${p.root?.emoji || ""} ${p.root} ""}`;
    st.style.color = p.root?.color;
    document.getElementById("ui-realm").textContent = PLAYER?.canhGioi;
    document.getElementById("ui-stage").textContent = `Tầng ${p.tuVi}`;
    ...
    document.getElementById("ui-tu").textContent =
      `${Math.floor(p.tuViLenCap)}/Chưa có`;
    document.getElementById("b-tu").style.width =
      Math.min(100, Math.floor((p.tuViLenCap / 100) * 100)) + "%";
    document.getElementById("ui-xu").textContent = p.xu?.toLocaleString();
    const s = p.stats;
    document.getElementById("r-str").textContent = s.str;
    document.getElementById("r-agi").textContent = s.agi;
    document.getElementById("r-vit").textContent = s.vit;
    document.getElementById("r-int").textContent = s.int;
    document.getElementById("r-patk").textContent = Combat.pAtk();
    document.getElementById("r-matk").textContent = 5 + s.int * 3;
    document.getElementById("r-def").textContent = Combat.pDef();
    document.getElementById("r-spd").textContent = 3 + Math.floor(s.agi * 0.5);
    document.getElementById("r-eva").textContent =
      Math.floor(s.agi * 0.3) + "%";
    document.getElementById("inv-head").textContent =
      `✦ TÚI ĐỒ (${PLAYER.iventoryIndex}/${40})`;
  },
  log(msg, cls = "") {
  ...
  buildInv() {
    ...
    for (let i = 0; i < CFG.INV_MAX; i++) {
      ...
      if (item) {
        ...
        sl.onclick = () => Inventory.use(i);
        sl.onmouseenter = (e) => UI.showTip(e, item);
        sl.onmouseleave = UI.hideTip;
      }
      grid.appendChild(sl);
    }
  },
  showTip(e, item) {
    ...
    document.getElementById("tip-body").innerHTML =
      `${item.desc || ""}<br><span style="color:var(--text2)">Bán: ${item.sell || 0} xu</span>`;
